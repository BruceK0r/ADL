...
class ThreeDomainFeatureEncoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if item_text_embeddings.shape[0] != config.item_num + 1:
        ...
        self.text_projection = nn.Sequential(
            nn.Linear(item_text_embeddings.shape[1], config.text_projection_dim),
            nn.LayerNorm(config.text_projection_dim),
            nn.ReLU(),
        )
        self.register_buffer("item_text_embeddings", item_text_embeddings, persistent=False)
        ...
        self.routing_dim = (
            config.user_dim + config.item_dim + 3 * config.text_projection_dim + config.time_dim
        )
    ...
    def _history_means(self, history: Tensor, domain: Tensor) -> tuple[Tensor, Tensor]:
        mask = history.ne(0)
        text = F.embedding(history, self.item_text_embeddings)
        global_denominator = mask.sum(dim=1, keepdim=True).clamp_min(1).to(text.dtype)
        global_mean = (text * mask.unsqueeze(-1)).sum(dim=1) / global_denominator

        history_domains = self.item_domains[history]
        domain_mask = mask & history_domains.eq(domain.unsqueeze(1))
        domain_denominator = domain_mask.sum(dim=1, keepdim=True).clamp_min(1).to(text.dtype)
        domain_mean = (text * domain_mask.unsqueeze(-1)).sum(dim=1) / domain_denominator
    ...
    def forward(
        ...
        candidate_text = self.text_projection(F.embedding(candidates, self.item_text_embeddings))
        ...
        routing = torch.cat(shared_parts, dim=-1).reshape(batch_size * candidate_count, -1)
